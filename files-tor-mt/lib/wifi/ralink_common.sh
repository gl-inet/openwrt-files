...
scan_ralink_wifi() {
    ...
    repair_wireless_uci
    [ ! -d "/tmp/$device" ] && mkdir /tmp/$device
    [ ! -f "/tmp/$device/$device.dat" ] && cp /etc/wireless/$device/$device.dat /tmp/$device/$device.dat
    #sync_uci_with_dat $device /etc/wireless/$device/$device.dat
    sync_uci_with_dat $device /tmp/$device/$device.dat
}
