...
function ap_openwrt_encryption_changed(form){
    var ap_encryption = form["ap_openwrt_encryption"];
	var ap_password = form["ap_openwrt_password"];
...
function ap_tor_encryption_changed(form){
    var ap_encryption = form["ap_openwrt_encryption"];
	var ap_password = form["ap_openwrt_password"];
	if(ap_encryption.value=="none"){
	    ap_password.placeholder="";
...
function networkCheck(form) {
	...
	var ap_openwrt_enable=form["ap_openwrt_enable"];
	var ap_openwrt_ssid = form["ap_openwrt_ssid"];
	var ap_openwrt_encryption = form["ap_openwrt_encryption"];
	var ap_openwrt_password = form["ap_openwrt_password"];
	...
	ap_openwrt_password.className = "normal";
	ap_openwrt_ssid.className = "normal";
	...
	if(ap_openwrt_enable.checked){
		if(nullOrEmpty(ap_openwrt_ssid.value)){
			errorHandler(ap_openwrt_ssid, errContainer, "SSID required");
			errors = true;
		}
		if(ap_openwrt_encryption.value=="wep" && ap_openwrt_password.placeholder != "Keep Unchanged"){
			if(ap_openwrt_password.value.length != 13 && ap_openwrt_password.value.length !=5){
				errorHandler(ap_openwrt_password, errContainer, "WiFi password must be 5 or 13 characters");
				errors = true;
			}
		}else if(ap_openwrt_encryption.value !="none" && ap_openwrt_password.placeholder != "Keep Unchanged"){
			if(ap_openwrt_password.value.length <8 && ap_openwrt_password.value !=""){
				errorHandler(ap_openwrt_password, errContainer, "WiFi password must be at least 8 characters");
				errors = true;
			}
		}
	}
...
function formCheck(form) {
  ...
  if (nullOrEmpty(hostname.value)) {
    errorHandler(hostname, errContainer, "Please choose a name for your Deivce");
    errors = true;

  } else if (hostname.value.match(/[^a-zA-Z0-9\-_]/)) {
    errorHandler(hostname, errContainer, "You can only use alphabetical characters for the hostname (A-Z, a-z, 0-9, - or _)");
    errors = true;
  }
...
function basicCheck(form) {
  ...
  if (nullOrEmpty(hostname.value)) {
    errorHandler(hostname, errContainer, "Please choose a name for your Device");
    errors = true;
...
function grey_out_ap_tor(disabled) {
  if (disabled) {
    document.getElementById("ap_tor_container").setAttribute("class", "disabled");
  } else {
    document.getElementById("ap_tor_container").setAttribute("class", "");
  }
  document.getElementById("ap_tor_password").disabled = disabled;
  document.getElementById("ap_tor_ssid").disabled = disabled;
  document.getElementById("ap_tor_encryption").disabled = disabled;
}

function grey_out_ap_openwrt(disabled) {
  if (disabled) {
    document.getElementById("ap_openwrt_container").setAttribute("class", "disabled");
  } else {
    document.getElementById("ap_openwrt_container").setAttribute("class", "");
  }
  document.getElementById("ap_openwrt_password").disabled = disabled;
  document.getElementById("ap_openwrt_ssid").disabled = disabled;
  document.getElementById("ap_openwrt_encryption").disabled = disabled;
}
...
document.body.onload = function() {
  ...
  var ap_openwrt_enable = document.getElementById("ap_openwrt_enable");
  if (ap_openwrt_enable) {
    ap_openwrt_enable.onclick = function(event) {
      grey_out_ap_openwrt(!event.target.checked);
    }
  }
