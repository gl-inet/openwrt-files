...
function index()
	function luci.dispatcher.authenticator.arduinoauth(validator, accs, default)
	  ...
	  make_entry({ "webpanel", "editor" }, nil, nil)
...
function homepage()
  ...
  local tor_log=luci.util.exec("logread |grep -E 'Tor.*Bootstrapped'|tail -n 1")
  local tor=tor_log:match("Bootstrapped ([^%s]+)%%")
...
--	local ap_disabled=uci:get("wireless",ap_name,"disabled")
	...
	local wifi_openwrt_disabled=uci:get_first("wireless","wifi-iface","disabled")
	--local wifi_tor_disabled=uci:get("wireless","tor","disabled")

	if(radio_disabled == "1" or wifi_openwrt_disabled == "1") then
		ap_openwrt_enabled=false
	end
--	if(radio_disabled == "1" or wifi_tor_disabled == "1") then
--		ap_tor_enabled=false
--	end
	...
	local ctx = {
		...
		wifi_openwrt = {
			enabled = ap_openwrt_enabled,
			mode = uci:get_first("wireless", "wifi-iface", "mode"),
...
--		wifi_tor = {
--			enabled = ap_tor_enabled,
--			mode = uci:get("wireless", "tor", "mode"),
--			ssid = uci:get("wireless", "tor", "ssid"),
--			encryption = uci:get("wireless", "tor", "encryption"),
--			--password = uci:get("wireless", ap_name, "key"),
--		},
		--timezones_wifi_reg_domains = timezones_wifi_reg_domains,
...
function network_post()
	...
	--setup wan------------------------
	local wan_protocol=param("wan_protocol")
	...
	elseif wan_protocol=="static" then
		...
		uci:set("network","wan","ifname",wan_eth)
		uci:set("network","wan","ipaddr",param("ipaddr"))
		uci:set("network","wan","netmask",param("netmask"))
		uci:set("network","wan","gateway",param("gateway"))
		uci:set("network","wan","dns",param("dns"))
	elseif wan_protocol=="pppoe" then
		uci:set("network","wan","proto","pppoe")
		uci:set("network","wan","ifname",wan_eth)
		uci:set("network","wan","username",param("username"))
		uci:set("network","wan","password",param("password"))
	elseif wan_protocol=="3g" then
		uci:set("network","wan","proto","3g");
		uci:set("network","wan","username",param("username"))
		uci:set("network","wan","password",param("password"))
		uci:set("network","wan","device",param("modem"))
		uci:set("network","wan","service",param("service"))
		uci:set("network","wan","pincode",param("pin"))
		uci:set("network","wan","apn",param("apn"))
	...
	if old_lan_ip ~= param("lan_ip") then
		uci:set("network","lan","ipaddr",param("lan_ip"))
		lan_ip_changed=true
		luci.util.exec("sed -i 's/192.168\..*\..*:/"..param("lan_ip")..":/g' /etc/tor/torrc")
	end

	--setup wifi openwrt----------------------
	if(param("ap_openwrt_enable")) then
		set_first(uci,"wireless","wifi-iface","disabled",'0')
		set_first(uci,"wireless","wifi-iface","ssid",param("ap_openwrt_ssid"))
		set_first(uci,"wireless","wifi-iface","encryption",param("ap_openwrt_encryption"))
		if param("ap_openwrt_encryption") == "wep" then
	        set_first(uci,"wireless","wifi-iface","key",'1')
	        if not_nil_or_empty(param("ap_openwrt_password")) then
		        set_first(uci,"wireless","wifi-iface","key1",param("ap_openwrt_password"))
	    	end
	    elseif param("ap_openwrt_encryption") == "none" then
	        delete_first(uci,"wireless","wifi-iface", "key")
	        delete_first(uci,"wireless","wifi-iface", "key1")
	    else
	        delete_first(uci,"wireless","wifi-iface", "key1")
	        if not_nil_or_empty(param("ap_openwrt_password")) then
	            set_first(uci,"wireless","wifi-iface","key",param("ap_openwrt_password"))
	        end
	    end
	else
		set_first(uci,"wireless","wifi-iface","disabled",'1')
	end
	----setup wifi tor ---
	--if(param("ap_tor_enable")) then
		--uci:set("wireless","tor","disabled",'0')
		--uci:set("wireless","tor","ssid",param("ap_tor_ssid"))
		--uci:set("wireless","tor","encryption",param("ap_tor_encryption"))
		--if param("ap_tor_encryption") == "wep" then
	        --uci:set("wireless","tor","key",'1')
	        --if not_nil_or_empty(param("ap_tor_password")) then
		        --uci:set("wireless","tor","key1",param("ap_tor_password"))
	    	--end
	    --elseif param("ap_tor_encryption") == "none" then
	        --uci:delete("wireless","tor", "key")
	        --uci:delete("wireless","tor", "key1")
	    --else
	        --uci:delete("wireless","tor", "key1")
	        --if not_nil_or_empty(param("ap_tor_password")) then
	            --uci:set("wireless","tor","key",param("ap_tor_password"))
	        --end
	    --end
		--luci.util.exec("sed -e '/^#.* wlan0 /s/^#//' -i /etc/firewall.user; /etc/init.d/firewall restart");
	--else
		--uci:set("wireless","tor","disabled",'1')
		----if tor ssid is disabled, we need to remove firewall rules
		--luci.util.exec("sed -e '/wlan/ s/^#*/#/' -i /etc/firewall.user; /etc/init.d/firewall restart");
	--end
...
--  local params = decrypt_pgp_message()
  ...
  if not_nil_or_empty(param("password")) then
  ...
  if not_nil_or_empty(param("hostname")) then
  ...
  if param("zonename") then
    ...
    local tz_regdomain = find_tz_regdomain(param("zonename"))
    if tz_regdomain then
      set_first(uci, "system", "system", "timezone", tz_regdomain.timezone)
      set_first(uci, "system", "system", "zonename", param("zonename"))
      delete_first(uci, "system", "system", "timezone_desc")
      --param("wifi.country") = tz_regdomain.code
    end
  end
...
--  local params = decrypt_pgp_message()
  ...
  if not_nil_or_empty(param("hostname")) then
    hostname = string.gsub(param("hostname"), " ", "_")
    set_first(uci, "system", "system", "hostname", hostname)
    uci:set("network", "lan", "hostname", hostname)
    uci:set("network", "wan", "hostname", hostname)
	--uci:set("network", "wwan", "hostname", hostname)
